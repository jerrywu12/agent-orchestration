...
Users choose a server folder using the in-app folder browser on every platform and Agent Desk discovers its repository. The native dialog is retired by the explicit [folder-selection resolution](folder-selection-resolution.md).
...
- FR04: In-app server folder chooser and inspection on every platform; preserve existing repository state and automatically reuse registered canonical roots. Selection/cancel must not require an OS dialog.
