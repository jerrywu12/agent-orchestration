...
4. An administrator may explicitly revoke a stale, untraceable claim after reviewing a fresh trace and clicking Confirm takeover. Per the user's [single-confirmation revision](simple-confirmation.md), there is no extra checkbox and notes are optional; the server supplies default audit text for omitted/blank notes. A verified live process cannot be taken over. Exact execution/session/heartbeat comparison fences races. Retain all old records and worktrees; fence late reports. Revocation does not kill unknown processes or claim to stop them.
