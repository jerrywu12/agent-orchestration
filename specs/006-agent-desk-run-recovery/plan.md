...
## Recovery contract
GET /api/tickets/:id/execution-status returns trace+lease+history. POST takeover requires executionId,sessionId,expectedHeartbeatAt,confirmed:true; reason is optional per the [single-confirmation revision](simple-confirmation.md), with default audit text for omitted/blank notes. Recheck trace and compare after asynchronous probes in transaction. Release as revoked with reason and retain original handles, mark held sessions revoked; no fake checkpoint assertion. Start separately with fresh claim/worktree and recovery context to avoid regenerating preserved code.
