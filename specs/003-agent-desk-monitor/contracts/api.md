...
Discovery amendment: MachineLibrary also permits ecosystem=plugin, status=cached for plugin-cache metadata. Preserve metadata version in this case, never reinterpret it as installed or enabled. Cached plugin rows require distinct UI labels/filter options.
