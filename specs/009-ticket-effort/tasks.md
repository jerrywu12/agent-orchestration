...
## US1 - Estimate effort
- [ ] T002 [US1] Add failing persistence/validation/session/metadata tests in `tools/agent-desk/tests/effort.test.mjs`, API tests in `tests/http.test.mjs`, MCP tests in `tests/mcp.test.mjs`, sync preservation in `tests/sync.test.mjs`.
- [ ] T003 [US1] Implement validation, defaults, public legacy read normalization and authorized mutations in `tools/agent-desk/server/service.mjs` and `bin/mcp.mjs`; run focused Node tests and commit.
- [ ] T004 [US1] Add failing create/edit/clear/reload DOM journey to `tools/agent-desk/tests/browser.spec.ts`.
- [ ] T005 [US1] Add `tools/agent-desk/src/effort.ts`, `src/components/EffortSelect.tsx` and update `src/types.ts`, `src/components/CreateDialogs.tsx`, `TicketDetails.tsx`; verify DOM journey.
