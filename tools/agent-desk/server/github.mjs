 ...
 * discoverWorkflowStatus(project) => {backlog,ready,active,review,done} option IDs.
 * projectStatusRole(project,status) => canonical role. These pure helpers require
 * unique exact canonical names after trim/collapse-whitespace/case normalization;
 * missing/ambiguous schemas and unknown item statuses throw nonretryable errors.
...
const MESSAGES = {
  ...
  NOT_FOUND:
    "GitHub resource is unavailable or not visible to the current identity.",
  UNAUTHENTICATED:
    "GitHub authentication is unavailable. Check the GitHub CLI identity.",
  FORBIDDEN:
    "GitHub denied this operation. Check repository and project permissions.",
  RATE_LIMITED:
    "GitHub rate limit reached. The pending operation can be retried later.",
  ...
  GRAPHQL_ERROR:
    "GitHub rejected the GraphQL operation. Check project access and configuration.",
  CONFLICT:
    "Local and GitHub edits conflict. Both snapshots are retained for resolution.",
  WRITE_NOT_APPLIED:
    "GitHub did not apply the requested update. Check permissions and sync again.",
  PAGINATION_LIMIT:
    "GitHub pagination exceeded the connector limit; import remains incomplete.",
  STATUS_FIELD_MISSING:
    "GitHub Project Status field is missing. Configure the canonical workflow in GitHub, then retry sync.",
  STATUS_OPTIONS_MISSING:
    "GitHub Project Status requires Backlog, Ready, In progress, In review and Done. Configure missing options in GitHub, then retry sync.",
  STATUS_OPTIONS_AMBIGUOUS:
    "GitHub Project Status options are ambiguous. Use one unique option per canonical workflow name, then retry sync.",
  UNKNOWN_PROJECT_STATUS:
    "GitHub Project item Status is empty, unknown or inconsistent. Select a canonical Status in GitHub, then retry sync.",
};

export class GitHubError extends Error {
  constructor(
    code,
    { status = null, retryable = false, retryAfterMs = 0, conflict } = {},
  ) {
...
const statusName = (value) =>
  typeof value === "string"
    ...
    : "";
export function discoverWorkflowStatus(project) {
  if (!project?.statusFieldId) throw new GitHubError("STATUS_FIELD_MISSING");
  const options = Array.isArray(project.statusOptions)
    ? project.statusOptions
    : [];
  const ids = new Set();
  for (const option of options) {
    if (!option?.id || ids.has(option.id))
      throw new GitHubError("STATUS_OPTIONS_AMBIGUOUS");
    ids.add(option.id);
  }
  return Object.fromEntries(
    WORKFLOW.map(({ name, role }) => {
      const matches = options.filter(
        (option) => statusName(option.name) === statusName(name),
      );
      if (!matches.length) throw new GitHubError("STATUS_OPTIONS_MISSING");
      if (matches.length !== 1)
        throw new GitHubError("STATUS_OPTIONS_AMBIGUOUS");
      return [role, matches[0].id];
    }),
  );
}
export function projectStatusRole(project, status) {
  const mapping = discoverWorkflowStatus(project);
  const match = Object.entries(mapping).find(
    ([, optionId]) => optionId === status?.optionId,
  );
  const option = project.statusOptions.find(
    (option) => option.id === status?.optionId,
  );
  if (
    !match ||
    status.fieldId !== project.statusFieldId ||
    (status.name !== undefined &&
      statusName(status.name) !== statusName(option.name))
  )
    throw new GitHubError("UNKNOWN_PROJECT_STATUS");
  return match[0];
}
...
function githubError(error = {}, { status, headers = {}, detail = "" } = {}) {
  ...
  headers = Object.fromEntries(
    Object.entries(error.headers ?? error.response?.headers ?? headers).map(
  ...
  if (!status)
    status = Number(diagnostic.match(/\bHTTP\s+(\d{3})\b/i)?.[1]) || null;
  const rateLimited =
    status === 429 ||
    /RATE_LIMITED|rate limit|secondary rate|abuse detection/i.test(
      diagnostic,
    ) ||
  ...
  if (rateLimited) {
    ...
    return new GitHubError("RATE_LIMITED", {
      status,
      retryable: true,
      retryAfterMs,
    });
  ...
  if (
    status === 401 ||
    /gh auth login|not logged into|requires authentication|bad credentials/i.test(
      diagnostic,
    )
  )
    return new GitHubError("UNAUTHENTICATED", { status });
  if (status === 403 || /FORBIDDEN|INSUFFICIENT_SCOPES/.test(diagnostic))
    return new GitHubError("FORBIDDEN", { status });
  if (status === 404 || /NOT_FOUND/.test(diagnostic))
    return new GitHubError("NOT_FOUND", { status });
  if (status === 408 || error.code === "ETIMEDOUT")
    return new GitHubError("TIMEOUT", {
      status,
      retryable: true,
      retryAfterMs: 5000,
    });
  ...
  if (error.code === "ENOENT" || error.code === "EACCES")
    return new GitHubError("CLI_UNAVAILABLE");
...
function parseResponse(output) {
  ...
  while (/^HTTP\/\S+ \d{3}/.test(text)) {
    ...
    for (const line of lines) {
      ...
      if (index > 0)
        headers[line.slice(0, index).toLowerCase()] = line
          .slice(index + 1)
          .trim();
    }
...
export function createGhRequest({
  ...
  return async (method, endpoint, body) => {
    if (
      ...
      !/^\/(?:repos\/|graphql$|user$)/.test(endpoint)
    )
      throw new GitHubError("INVALID_INPUT");
    const args = [
      "api",
      "--hostname",
      "github.com",
    ...
    return new Promise((resolve, reject) => {
      ...
      timer = setTimeout(
        () =>
          stop(
            new GitHubError("TIMEOUT", { retryable: true, retryAfterMs: 5000 }),
          ),
        timeoutMs,
      );
      const collect = (stream) => (chunk) => {
        ...
        if (byteCount > maxBytes) {
          stop(new GitHubError("OUTPUT_LIMIT"));
          return;
        }
        ...
        try {
          ...
          if (code !== 0 || response.status >= 400) {
            finish(
              githubError(
                {},
                { ...response, detail: `${stderr} ${response.text}` },
              ),
            );
            return;
          }
          ...
          finish(new GitHubError("INVALID_RESPONSE"));
        }
      });
      try {
        child.stdin.end(body === undefined ? undefined : JSON.stringify(body));
      } catch {
        stop(new GitHubError("INVALID_INPUT"));
      }
    });
  };
}
...
function readBody(body) {
  ...
  try {
    return {
      ...
      agentDeskId: decodeURIComponent(match[1]),
    };
  } catch {
    return { description: body, agentDeskId: null };
  }
}
...
export function issueSnapshot(source = {}) {
  ...
  if (own(source, "ownerId")) {
    ...
    if (
      ...
      source.ownerId &&
      source.ownerId !== "unassigned"
    )
      labels.push(`owner:${source.ownerId}`);
  }
  return {
    title: string(source.title),
    description: own(source, "description")
      ? string(source.description)
      : readBody(source.body).description,
    labels: unique(labels),
    state:
      String(source.state ?? source.github?.state ?? "open").toLowerCase() ===
      "closed"
        ? "closed"
        : "open",
  };
...
function comparison(snapshot) {
  return { ...snapshot, labels: snapshot.labels.filter(managed) };
}

export function reconcileIssue({ local, remote, baseline }) {
  local = issueSnapshot(local);
  remote = issueSnapshot(remote);
  baseline = baseline ? issueSnapshot(baseline) : null;
  const left = comparison(local);
  const right = comparison(remote);
  const previous = baseline && comparison(baseline);
  const localChanges = previous
    ? SYNC_FIELDS.filter((field) => !same(left[field], previous[field]))
    : [];
  const remoteChanges = previous
    ? SYNC_FIELDS.filter((field) => !same(right[field], previous[field]))
    : [];
  const conflicts = localChanges.filter(
    (field) =>
      remoteChanges.includes(field) && !same(left[field], right[field]),
  );
  const result = {
    kind: "unchanged",
    local,
  ...
  for (const field of localChanges) {
    merged[field] =
      field === "labels"
        ? unique([
            ...remote.labels.filter((label) => !managed(label)),
            ...left.labels,
          ])
        : local[field];
  }
  return {
    ...result,
    kind: same(comparison(merged), right) ? "remote" : "local",
    merged,
  };
}
...
function validateEdits(edits) {
  ...
  if (
    ...
    (typeof edits.title !== "string" ||
      !edits.title.trim() ||
      edits.title.length > 256)
  )
    throw new GitHubError("INVALID_INPUT");
  if (
    own(edits, "description") &&
    (typeof edits.description !== "string" || edits.description.length > 65536)
  )
    throw new GitHubError("INVALID_INPUT");
  if (
    own(edits, "labels") &&
    (!Array.isArray(edits.labels) ||
      edits.labels.length > 100 ||
      edits.labels.some(
  ...
  if (
    own(edits, "ownerId") &&
    edits.ownerId !== null &&
    (typeof edits.ownerId !== "string" || edits.ownerId.length > 44)
  )
    throw new GitHubError("INVALID_INPUT");
  if (own(edits, "state") && !["open", "closed"].includes(edits.state))
    throw new GitHubError("INVALID_INPUT");
}
...
function normalizeIssue(repo, value) {
  if (
    ...
    throw new GitHubError("INVALID_RESPONSE");
  if (own(value, "pull_request")) throw new GitHubError("NOT_ISSUE");
  return {
    repo,
    number: value.number,
    nodeId: value.node_id ?? value.nodeId ?? value.id,
    url: value.html_url ?? value.url ?? null,
    ...issueSnapshot(value),
    updatedAt: value.updated_at ?? value.updatedAt ?? null,
    agentDeskId: value.agentDeskId ?? readBody(value.body).agentDeskId,
  };
}
...
export class GitHubClient {
    ...
    try {
      ...
      if (
        ...
        const detail = result.errors
          .map(
            (error) =>
              `${error.type ?? error.extensions?.code ?? ""} ${error.message ?? ""}`,
          )
          .join(" ");
  ...
  async listIssues(repo) {
    ...
    for (let page = 1; page <= MAX_PAGES; page++) {
      const rows = await this.call(
        "GET",
        `/repos/${repo}/issues?state=all&sort=created&direction=asc&per_page=100&page=${page}`,
      );
  ...
  async getIssue(repo, number) {
    return normalizeIssue(
      validateRepo(repo),
      await this.call("GET", `/repos/${repo}/issues/${validateNumber(number)}`),
    );
  }
  ...
  async createMarkedIssue(repo, ticket) {
    const findExisting = async () =>
      (await this.listIssues(repo)).find(
        (issue) => issue.agentDeskId === ticket.id,
      );
    ...
    const body = {
      title: desired.title,
      body: desired.description + marker(ticket.id),
      labels: unique(["agent-desk", ...desired.labels.filter(managed)]),
    };
    try {
      return normalizeIssue(
        repo,
        await this.call("POST", `/repos/${repo}/issues`, body),
      );
    } catch (error) {
      if (
        error.retryable ||
        ["INVALID_RESPONSE", "OUTPUT_LIMIT"].includes(error.code)
      ) {
        try {
          const recovered = await findExisting();
          if (recovered) return recovered;
        } catch {
          /* Preserve the original classified failure for durable retry. */
        }
      }
      throw error;
    }
  ...
  async updateIssue(repo, number, edits, baseline) {
    ...
    const local = {
      ...issueSnapshot(baseline ?? remote),
      ...Object.fromEntries(
        Object.entries(edits).filter(([key]) =>
          [...SYNC_FIELDS, "ownerId"].includes(key),
        ),
      ),
    };
    const reconciliation = reconcileIssue({
      local,
      remote,
      baseline: baseline ?? remote,
    });
    if (reconciliation.kind === "conflict")
      throw new GitHubError("CONFLICT", { conflict: reconciliation });
    const desired = reconciliation.merged;
    const patch = {};
    for (const field of ["title", "description", "state"]) {
      if (desired[field] !== remote[field])
        patch[field === "description" ? "body" : field] =
          field === "description"
            ? desired.description +
              (remote.agentDeskId ? marker(remote.agentDeskId) : "")
            : desired[field];
    }
    const endpoint = `/repos/${repo}/issues/${number}`;
    if (Object.keys(patch).length) await this.call("PATCH", endpoint, patch);
    const added = desired.labels.filter(
      (label) => managed(label) && !remote.labels.includes(label),
    );
    const removed = remote.labels.filter(
      (label) => managed(label) && !desired.labels.includes(label),
    );
    // Individual label operations never replace unrelated labels, including races.
    if (added.length)
      await this.call("POST", `${endpoint}/labels`, { labels: added });
    ...
    if (!Object.keys(patch).length && !added.length && !removed.length)
      return remote;
    const updated = await this.getIssue(repo, number);
    // GitHub can silently ignore fields for identities lacking write permission.
    if (!same(comparison(issueSnapshot(updated)), comparison(desired)))
      throw new GitHubError("WRITE_NOT_APPLIED");
    return updated;
  }
    ...
    for (let page = 0; page < MAX_PAGES; page++) {
      ...
      for (const field of project.fields.nodes) {
        if (field?.name === "Status" && Array.isArray(field.options)) {
          ...
          result.statusOptions = field.options.map(({ id, name }) => ({
            id,
            name,
          }));
        }
    ...
    for (let page = 0; page < MAX_PAGES; page++) {
      ...
      for (const item of data.node.items.nodes) {
        ...
        items.set(item.id, {
          id: item.id,
          issue,
          status: item.status?.optionId
            ? {
                fieldId: item.status.field.id,
  ...
  async setProjectStatus(
    ...
    if (itemId) validateId(itemId);
      ...
      const data = await this.graphql(
        `mutation AgentDeskAddItem($input:AddProjectV2ItemByIdInput!) { addProjectV2ItemById(input:$input) { item { id } } }`,
    ...
    const data = await this.graphql(
      `mutation AgentDeskStatus($input:UpdateProjectV2ItemFieldValueInput!) { updateProjectV2ItemFieldValue(input:$input) { projectV2Item { id } } }`,
      {
        input: {
          projectId,
          itemId,
  ...
  async getPullRequest(repo, number) {
    ...
    if (!value || value.number !== number || typeof value.merged !== "boolean")
      throw new GitHubError("INVALID_RESPONSE");
    const merged =
      value.merged === true &&
    ...
    return {
      ...
      state: value.state,
      merged,
      mergedAt: merged ? value.merged_at : null,
      headSha: value.head?.sha ?? null,
      mergeCommitSha: merged ? (value.merge_commit_sha ?? null) : null,
    };
  }
  ...
  async check() {
    try {
      ...
      return { available: false, error: githubError(error).message };
    }
  }
}
